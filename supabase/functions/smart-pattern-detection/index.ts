...
async function detectPatterns(driver: any) {
  ...
  try {
    ...
    `)
    const totalSummaries = toNumber(getValue(summaryCount.records[0], 'count'))
    console.log(`Total summaries: ${totalSummaries}`)
    
    if (totalSummaries === 0) {
      console.log('No summaries to analyze for patterns')
      return patterns
    }
    
    // Simple debugging pattern detection
    ...
    for (const record of debugResult.records) {
      const debugCount = toNumber(getValue(record, 'debugCount'))
      if (debugCount > 5) {
        patterns.push({
          type: 'debugging',
    ...
    for (const pattern of patterns) {
      ...
      `, {
        ...
        metadata: JSON.stringify({
          project: pattern.project || 'unknown',
          day: pattern.day ? pattern.day.toString() : 'unknown',
