...
serve(async (req) => {
  ...
  try {
    ...
    await logger.info('Memory ingestion coordinator started', { batchId, workerCount: WORKER_COUNT })

    // Check if there are messages in the queue
    const { data: queueStatus, error: statusError } = await supabase.rpc('pgmq_metrics', {
      p_queue_name: 'memory_ingestion'
    })
    ...
    if (queueLength === 0) {
      await logger.info('No messages in memory ingestion queue')
      await dbLogger.close()
      return new Response(JSON.stringify({ 
    ...
    for (let i = 0; i < WORKER_COUNT; i++) {
      ...
      const workerPromise = fetch(`${supabaseUrl}/functions/v1/memory-ingestion-worker`, {
        method: 'POST',
        headers: {
      ...
      await logger.info(`Spawned worker ${i} with ${workerMessages.length} messages`, { 
        workerId,
    ...
    return new Response(
      JSON.stringify({ 
        message: `Processed ${totalProcessed} memories with ${totalErrors} errors`,
    ...
    if (dbLogger) {
      await logger.error('Coordinator fatal error', error)
      await dbLogger.close()
    }
    return new Response(
      JSON.stringify({ 
