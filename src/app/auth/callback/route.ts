...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (!error) {
      ...
      if (isCliAuth) {
        console.log('[Auth Callback] CLI auth detected, processing...')
        ...
        try {
          ...
          console.log('[Auth Callback] Got user:', { id: user.id, email: user.email })
          ...
          if (existingKey) {
            console.log('[Auth Callback] User already has a Camille CLI key')
            action = 'existing'
            apiKey = '' // Don't expose existing keys
          } else {
            console.log('[Auth Callback] Creating new API key...')
            const result = await createApiKey(user.id, 'Camille CLI')
            
            if (result.error) {
              console.error('[Auth Callback] Failed to create API key:', result.error)
              throw new Error('Failed to create API key')
            }
            
            console.log('[Auth Callback] API key created successfully')
            ...
            <html>
              ...
              <body>
                ...
                <script>
                  ...
                  (async () => {
                    console.log('[CLI Auth] Attempting to send API key to CLI at http://localhost:${cliPort}/cli-callback');
                    try {
                      const response = await fetch('http://localhost:${cliPort}/cli-callback', {
                        method: 'POST',
                        headers: {
